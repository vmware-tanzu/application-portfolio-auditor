...
function display_duration() {
	...
	while read -r FILE; do
		APP_GROUP=$(dirname "${FILE}" | rev | cut -d '_' -f 1 | rev)
		echo ">>>>>>>> ${APP_GROUP} - ${FILE}"
		...
		date_str1=$(grep -o '\[[0-9_]*\]' "${TMP_FILE}" | sed -n 1p | awk -F'[_\\[\\]]' '{print $2"-"$3"-"$4"__"$6"_"$7"_"$8}')
		date_str2=$(grep -o '\[[0-9_]*\]' "${TMP_FILE}" | sed -n 2p | awk -F'[_\\[\\]]' '{print $2"-"$3"-"$4"__"$6"_"$7"_"$8}')

		if [[ -n "${date_str1}${date_str2}" ]]; then
			...
			# Print the result
			echo -e "${APP_GROUP} - ${GREEN}Duration: $HOURS hours, $MINUTES minutes, $SECONDS seconds${NORMAL} >>> ${BLUE}$(dirname "${FILE}")/index.html${NORMAL}"
			(( TOTAL_HOURS += HOURS ))
			(( TOTAL_MINUTES += MINUTES ))
			(( TOTAL_SECONDS += SECONDS ))
		else
			echo -e "${APP_GROUP} - ${RED}No time data found${NORMAL} >>> ${BLUE}$(dirname "${FILE}")/index.html${NORMAL}"
		fi
		rm -f "${TMP_FILE}"

	done < <(find "${REPORT_FOLDER}" -mindepth 2 -maxdepth 2 -type f -name '*_timeline.log')
